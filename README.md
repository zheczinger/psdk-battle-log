...
| **`hp_measurement`** | "percent",<br/>"points",<br/>"both" | "percent" | How health lost or restored is reported in the battle log. The choices are `"percent"` (percentage of the battler's max health), `"points"` (exact HP quantity), and `"both"`. |
...
- [Modernized Battle Messages for PSDK](https://github.com/zheczinger/modernized-battle-messages): Compatible with Version v0.5.4 or newer of that plugin. Battle messages replaced or silenced by that plugin are preserved in this plugin's battle log.
